...
function gotoSurvey(){
  ...
  if (document.getElementById("checkbox_SecurityPractices").checked && 
      document.getElementById("checkbox_DataCollected").checked &&
      document.getElementById("checkbox_DataShared").checked) {
      ...
      var gotoSurveyButton = document.getElementById("gotoSurvey");
      gotoSurveyButton.style.display = "none";
      

  } else {
    alert("You must scroll down and click on all checkboxs in the second interface in order to proceed");
  }

}

function displayDetails(){
  var details = document.getElementById("detailed_contents");
...
function nextChannel(){
  ...
  var currentSequence = channelOrder.indexOf('data_safety');
...
function submit(){
  ...
  localStorage.setItem('DS-C1', C1_answer);
  localStorage.setItem('DS-C2', C2_answer);

  localStorage.setItem('DS-C3', JSON.stringify(C3_answer));
  if (C3_answer.includes("Others")){
     localStorage.setItem('DS-C3_other', C3_others);
  }
  localStorage.setItem('DS-C4', JSON.stringify(C4_answer));
  if (C4_answer.includes("Others")){
     localStorage.setItem('DS-C4_other', C4_others);
  }
  localStorage.setItem('DS-C5', JSON.stringify(C5_answer));
  if (C5_answer.includes("Others")){
     localStorage.setItem('DS-C5_other', C5_others);
  }
  localStorage.setItem('DS-C6', JSON.stringify(C6_answer));
  if (C6_answer.includes("Others")){
     localStorage.setItem('DS-C6_other', C6_others);
  }
  localStorage.setItem('DS-C7', JSON.stringify(C7_answer));
  if (C7_answer.includes("Others")){
     localStorage.setItem('DS-C7_other', C7_others);
  }



  localStorage.setItem('DS-C8', C8_answer);
  localStorage.setItem('DS-C9', C9_answer);
  localStorage.setItem('DS-C10', C10_answer);
  localStorage.setItem('DS-C11', C11_answer);
  localStorage.setItem('DS-C12', C12_answer);
  localStorage.setItem('DS-C12-open-question', C12_answer_open);
  localStorage.setItem('DS-C13', C13_answer);
  localStorage.setItem('DS-C13-open-question', C13_answer_open);
